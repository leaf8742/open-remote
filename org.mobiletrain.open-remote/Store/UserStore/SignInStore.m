...
- (void)requestWithSuccess:(void (^)())success failure:(void (^)(NSError *error))failure {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        [manager POST:[URLString stringByAppendingString:@"user/sign_in"] parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
        } success:^(NSURLSessionDataTask *task, id responseObject) {
            responseError = [BaseStore errorWithResponseObject:responseObject];
            if (responseError == nil) {
                // 服务器返回成功，设置用户模型单例
                UserModel *user = [UserModel userWithIdentity:responseObject[@"user_id"]];
                ...
                [UserListModel sharedInstance].currentUser = user;
            }
